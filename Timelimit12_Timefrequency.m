...
%                     cfg.gwidth= 3;
                    ...
                    TFR{condi} = ft_freqanalysis(cfg,DATA_CLEAN);
                   ...
                   disp('Save TIMEFREQ AVERAGES results? RISK OF OVERWRITING... ')
                        ...
                        filename= [sprintf('subj%02d_TFR_one', subi)]; % add one if all trials mixed by condition
                        save(filename,'TFR','-v7.3'); 
...
for subi=1:nSubjs;
    ...
    fname_TFR= sprintf('subj%02d_TFR',subi);
...
for subi= 1:nSubjs; %nGoodSubjects
    ...
    for k= 1:5
        TFRmatrix{subi,k}= pickupTFR(subi).TFR{k}; %pickupSub(i).avg_EEG{k}
    end
end
...
cfg.showlabels= 'yes';
figure
ft_multiplotTFR(cfg,Grand_TFR{5});%Grand_TFR{2},Grand_TFR{3},Grand_TFR{4},Grand_TFR{5}
...
% cfg.statistic = 'ft_statfun_indepsamplesF'; %'ft_statfun_depsamplesT';
cfg.design = design;
cfg.ivar = 1;
cfg.uvar = 2;
